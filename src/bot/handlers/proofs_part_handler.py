...
enter_proofs_btn: KeyboardButton = KeyboardButton(f"{emojis.pencil} ввести самостоятельно")
...
async def proofs_start(message: types.Message):
    ...
    await message.reply("Укажите доказательства, которые подтверждают нарушение ваших прав. "
                        "Выберите одну из следующий опций. Или укажите свой вариант",
...
async def show_example(message: types.Message, state: FSMContext):
    ...
    examples: Optional[List[str]] = repository.get_claim_tmp_examples(claim_theme, "proofs")
